...
function MainPage() {
  ...
  useEffect(()=>{
    ...
    axios.get(`https://product-portal-rnaz.onrender.com/get-prods?category=${category.join(',')}`)
          ...
          <div className="product-cards">
            {
              sortedProducts.map((product, key)=>(
                <div className='main-card-container-div' style={{display:'flex', flexDirection:'column', backgroundColor:'#36416A26',marginBottom:'1rem',borderRadius:'0.5rem', padding:'0.5rem 1rem 0 1rem'}}>
                  <div className='wrapped-products' style={{...productCardsStyles}}>
                    <div className='logo' style={{width:'10%', }}> < img className='logo-img' src={product.logoURL} alt="image" height={70} /> </div>
                    <div style={{display:'flex', justifyContent:'space-between', width:"90%"}}>
                      <div className="card-main-info">
                        <h2 className='company-name' style={{marginTop:0, marginBottom:'0.5rem', color:'#36416A'}}> <a href={product.productLink}></a> {product.companyName}</h2>
