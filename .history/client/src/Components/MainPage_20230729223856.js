        ...
        <div className='navbar' >
          ...
          <div style={{display:'flex', marginRight:'5rem'}}>
              {isLoggedIn ? (
                ...
                <>
                  <button className='btn-main-page-login' type='submit'  onClick={() => setShowLoginPopup(true)} >Log in</button>
                  <button type='submit'  className='btn-main-page-register' onClick={()=>{navigate('/signup')}} >Sign up</button>
                </>
              )}
              {showLoginPopup && <LoginPopUp closeModal={() => setShowLoginPopup(false)} />}
            </div>
          </div>
            <LoginPopUp/>
        </div>
