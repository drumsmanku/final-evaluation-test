...
function SignupPopUp({closeModal, showSignupModal,setShowLoginModal, setShowLoginModalFromSignup}) {
  ...
  const signup = (event) => {
    ...
    axios.post('https://product-portal-rnaz.onrender.com/signup', user)
      .then(res => {
          ...
          <div className="modal-signup" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }} onClick={handleClickOutside}> 
            <div className="modal-content" onClick={handleClickInside}> 
              ...
              <h2>Signup to continue</h2>
                ...
                <span style={{color:'#737373'}}>Already have an account?<button style={{background:'none', color:'#36416A', fontSize:'medium', marginLeft:0, padding:0, width:'4rem', cursor:'pointer', border:'none', textDecoration:'underline'}}   onClick={() => { 
                      closeModal();
